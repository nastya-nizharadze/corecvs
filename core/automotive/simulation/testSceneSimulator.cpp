...
std::vector<Vector3dd> *TestSceneSimulator::generateMockScene()
{
...
void TestSceneSimulator::unitCube(unsigned gradations, std::vector<Vector3dd> &output)
{
    //vector<Vector3dd> *pointsIn3d = new std::vector<Vector3dd>();
    double step = 1.0 / (gradations - 1);
    output.reserve(gradations * gradations * gradations);
...
std::vector<Vector3dd> *TestSceneSimulator::applyTransform(std::vector<Vector3dd> * pointsA, const Matrix44 &A)
{
...
std::vector<FloatFlowVector> *TestSceneSimulator::simulateFlow(
        ...
        double minLength )
{
    ...
    vector<Vector3dd> *pointsA = TestSceneSimulator::generateMockScene();
    vector<Vector3dd> *pointsB = TestSceneSimulator::applyTransform(pointsA, A);

    Matrix44 D = FCostFunction::getDMatrix(FCostFunction::FRONT_VIEW_CAMERA, camIntrinsics, cameraExtrinsics);

    vector<Vector3dd> *imagePointsA = TestSceneSimulator::applyTransform(pointsA, D);
    vector<Vector3dd> *imagePointsB = TestSceneSimulator::applyTransform(pointsB, D);
