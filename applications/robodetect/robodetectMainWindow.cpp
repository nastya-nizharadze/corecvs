...
RobodetectMainWindow::RobodetectMainWindow(QWidget *parent)
    : QMainWindow(parent)
    , mUi(new Ui::RobodetectMainWindowClass)
    , mImageWidget(NULL)
    , mImage(NULL)
    , mMask(NULL)
    , mMaskBlended(NULL)
    ...
    , mEdges(NULL)
{
    ...
    mImageWidget = new RobodetectImageWidget(this);
...
void RobodetectMainWindow::radiusChanged()
{
    mImageWidget->decorationSize = (double)mUi->fingerSizeSpinBox->value() * (12.8 / 4 * 3);
}


void RobodetectMainWindow::closeEvent(QCloseEvent *event)
{
    Q_UNUSED(event)
    qApp->quit();
}
...
void RobodetectMainWindow::connectActions()
{
...
void RobodetectMainWindow::keyPressEvent(QKeyEvent * event)
{
...
void RobodetectMainWindow::loadImage(void)
{
...
G8Buffer *RobodetectMainWindow::projectToDirection(RGB24Buffer *input, const Vector3dd &direction)
{
...
void RobodetectMainWindow::preprocessImage(void)
{
    if (mImage == NULL) {
        return;
    }
...
void RobodetectMainWindow::toggleMask(void)
{
    updateViewImage();
}

void RobodetectMainWindow::resetMask(void)
{
...
void RobodetectMainWindow::undoMask(void)
{
    if (mUndoList.empty()) {
        return;
    }
...
void RobodetectMainWindow::prepareBlendedMask()
{
...
void RobodetectMainWindow::updateViewImage(void)
{
...
void RobodetectMainWindow::maskHue(int hue1, int hue2)
{
    for (int i = 0; i < mImage->h; i++)
    {
       for (int j = 0; j < mImage->w; j++)
...
void RobodetectMainWindow::recursiveTolerance(RGBColor startColor, int tolerance, int x, int y)
{
    if (!mMask->isValidCoord(y,x))
        return;
    if (mMask->element(y,x))
...
void RobodetectMainWindow::maskTolerance(QPoint point)
{
    int x = point.x();
    int y = point.y();
...
void RobodetectMainWindow::maskTolerance1(QPoint point)
{
    int x = point.x();
    int y = point.y();
    L_INFO_P("Starting the fill");
...
void RobodetectMainWindow::maskToleranceFinal(QPoint point)
{
    int x = point.x();
    int y = point.y();
    L_INFO_P("Starting the fill");
...
void RobodetectMainWindow::maskToleranceGraph(QPoint point)
{
    int x = point.x();
    int y = point.y();
    L_INFO_P("Starting the fill");
...
void RobodetectMainWindow::maskLocalHist(QPoint point)
{
    int x = point.x();
    int y = point.y();
    L_INFO_P("Starting the fill");
...
void RobodetectMainWindow::maskLocalHist2D(QPoint point)
{
    int x = point.x();
    int y = point.y();
    L_INFO_P("Starting the fill");
...
void RobodetectMainWindow::doMask(QPoint point, int id)
{
    qDebug() << "RobodetectMainWindow::doMask(): Point Selected(" << point << "," << id << ")";
...
void RobodetectMainWindow::doFill()
{
...
void RobodetectMainWindow::paramChanged()
{
    qDebug("RobodetectMainWindow::paramChanged(): called\n");
}

void RobodetectMainWindow::doErease(QPoint point)
{
    mUndoList.push_back(new G8Buffer(mMask));
    if (mUndoList.size() > 20) {
        delete_safe(mUndoList.front());
...
void RobodetectMainWindow::doFillGapsMorpho()
{
...
void RobodetectMainWindow::doFillGapsMorphoFast()
{
    int size = mUi->fillCloseSizeSpinBox->value();
    L_INFO_P("Size %d", size);
...
void RobodetectMainWindow::doFillGapsSegment()
{
...
void RobodetectMainWindow::doUpdateLocalHist(QPoint point)
{
    int x = point.x();
    int y = point.y();
...
void RobodetectMainWindow::doUpdateLocalHist2D(QPoint point)
{
    int x = point.x();
    int y = point.y();
...
void RobodetectMainWindow::pointSelected(int toolID, QPoint point)
{
    switch (toolID)
    {
        case MASK_TOOL:
...
void RobodetectMainWindow::pointSelectedMoved(int toolID, QPoint point)
{
    switch (toolID)
    {
        case EREASE_TOOL:
...
void RobodetectMainWindow::rectSelected(int /*toolID*/, QRect rect)
{
...
RobodetectMainWindow::~RobodetectMainWindow()
{
